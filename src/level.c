...
void level_load(level *level, SDL_Renderer *renderTarget)
{
   ...
   level->textures[0] = texture_load("src/images/flower.png", renderTarget);
   animator_create(level->animations[0], level->textures[0], 16, 16, 2);

   // grass
   level->animations[1] = (animator*) malloc(sizeof(animator));
   level->textures[3] = texture_load("src/images/grass.png", renderTarget);
   animator_create(level->animations[1], level->textures[3], 16, 16, 4);

   // heart
   level->animations[2] = (animator*) malloc(sizeof(animator));
   level->textures[5] = texture_load("src/images/heart.png", renderTarget);
   animator_create(level->animations[2], level->textures[5], 32, 32, 4);

   // water
   level->animations[3] = (animator*) malloc(sizeof(animator));
   level->textures[7] = texture_load("src/images/water.png", renderTarget);
   animator_create(level->animations[3], level->textures[7], 16, 16, 4);

   // bigmama
   level->animations[4]  = (animator*) malloc(sizeof(animator));
   level->textures[8] = texture_load("src/images/bigmama.png", renderTarget);
   animator_create(level->animations[4], level->textures[8], 32, 32, 2);

   // birthday
   level->textures[1] = texture_load("src/images/birthday.png", renderTarget);

   // ground
   level->textures[2] = texture_load("src/images/ground.png", renderTarget);

   // wall
   level->textures[4] = texture_load("src/images/wall.png", renderTarget);

   // rock
   level->textures[6] = texture_load("src/images/rock.png", renderTarget);
}
